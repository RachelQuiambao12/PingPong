...
pygame.display.set_caption('PingPongPingPong')
white = [255, 255, 255]
black = [0, 0, 0]
red = [255, 0, 0]
blue = [0, 0, 255]
...
class Menu():
	...
	def __init__(self, pos, data=[], textsize=32,\
			wordcolor=[255, 0, 255], backcolor=[0, 0, 255], selectedcolor=[0, 255, 255], lprn=20):
		#import pygame
...
def newGame(twoplayer=False):
	...
	class Ball():
		...
		def update(self, paddle, enemy):
			global screen, black, phase, spap, die
...
def gameLoop(paddle, enemy, ball, twoplayer, paddle_two):
	global screen, black, black, clock
	paddlescore = pygame.font.Font(None, 32)
	enemyscore = pygame.font.Font(None, 32)
	topscore = 0
	screen.fill(black)
	screen.blit(paddlescore.render('Ready!!!', True, red), ball.center)
	pygame.display.flip()
	time.sleep(3)
	screen.fill(black)
	screen.blit(paddlescore.render('Get Set', True, red), ball.center)
	pygame.display.flip()
	time.sleep(1)
	screen.fill(black)
	screen.blit(paddlescore.render('GO!', True, red), ball.center)
	pygame.display.flip()
	time.sleep(0.5)
	while not topscore > 2:
		...
		topscore = topscore[0]
		screen.blit(paddlescore.render(str(paddle.score), True, black), [230, 0])
		if twoplayer:
			screen.blit(enemyscore.render(str(paddle_two.score), True, white), [360, 0])
		else:
			screen.blit(enemyscore.render(str(enemy.score), True, black), [460, 0])
	...
	if not twoplayer:
		if paddle.score == topscore:
			screen.blit(paddlescore.render('You Won!', True, blue), [250, 250])
			pygame.display.flip()
			while True:
				for event in pygame.event.get():
			...
			screen.blit(paddlescore.render('GAME OVER!!', True, red), [250, 250])
			pygame.display.flip()
			while True:
				for event in pygame.event.get():
					if event.type == pygame.KEYDOWN:
